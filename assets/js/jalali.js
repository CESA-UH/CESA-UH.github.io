(function () {
  ...
  function toPersianDigits(num) {
    return String(num).replace(/[0-9]/g, function (d) { return FA_DIGITS[d]; });
  }

  function formatScheduleDate(iso, lang) {
    if (!iso) return "";
    var parts = iso.split("-").map(Number);
    var gy = parts[0], gm = parts[1], gd = parts[2];
    var weekdayIndex = new Date(gy, gm - 1, gd).getDay();

    if (lang === "fa") {
      var j = toJalaali(gy, gm, gd);
      return FA_WEEKDAYS[weekdayIndex] + "، " + toPersianDigits(j.jd) + " " + FA_MONTHS[j.jm - 1] + " " + toPersianDigits(j.jy);
    }
